...
def moduless(request):
    ...
    if request.method == "POST":
        ...
        if model == "1":
            ...
            ac
            msg1 ='Accuracy of RandomForest : ' + str(ac*100)
            return render(request,'moduless.html',{'msg':'msg1'})
        elif model == "2":
            de = DecisionTreeClassifier(criterion= 'entropy',max_depth=2)
            de.fit(x_train,y_train)
            de_pred = de.predict(x_test)
            ac1 = accuracy_score(y_test,de_pred)
            ac1
            msg='Accuracy of Decision tree : ' + str(ac1*100)
            return render(request,'moduless.html',{'msg':'msg2'})
        elif model == "3":
            ...
            bc
            msg='Accuracy of GradientBoostingClassifier : ' + str(bc*100)
            return render(request,'moduless.html',{'msg':'msg3'})
        elif model == "4":
            model1 =  RandomForestClassifier()
            model2 = DecisionTreeClassifier()
            st =  StackingClassifier(estimators=[('ra',model1),('de',model2)],final_estimator=GradientBoostingClassifier())
            st.fit(x_train,y_train)
            st_pred = st.predict(x_test)
            sc = accuracy_score(y_test,st_pred)
            msg='Accuracy of StackingClassifier : ' + str(sc*100)
            return render(request,'moduless.html',{'msg':'msg4'})
